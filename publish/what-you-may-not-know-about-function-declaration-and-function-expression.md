...
在代码执行阶段，当解释器遇到一个具名的函数表达式，它的算法描述如下：
...
另外顺带一提，在被调用的时候，会把 `[[Scope]]` 上的作用域链又以同样的方式浅拷贝到新创建的上下文中，并添加新上下文的变量对象到新上下文的作用域链的前端。所以不是搜索 `[[Scope]]` 上的作用域链哦，而是新上下文的作用域链。详细说明，请等待我将要发布的系列文章。
